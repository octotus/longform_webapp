...
| 1.3 | Open DevTools → Application → Local Storage | Key `longform_db` exists after first load | [P] | |
| 1.4 | Open DevTools → Application → Local Storage | Key `longform-settings` exists | [P] | |
| 1.5 | Verify the Library page is shown as the default route (`/`) | Library page renders with header "Longform" | [P] | |
...
| # | Step | Expected Result | Status | Notes |
|---|------|-----------------|--------|-------|
| 2.1 | Observe header | Shows: "Longform" title, Global Refs, Corpus, Settings buttons, "+ New Article" button | [P] | |
...
| 32.6 | Check `longform_db` key | Present and non-empty | [P] | |
| 32.7 | `longform_db` value | Long base64 string | [P] | |
...
*Test run completed: 2026-03-14 · Commit `a397a9f` · longform_webapp*
