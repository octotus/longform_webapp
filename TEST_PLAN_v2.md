...
| 1.3 | DevTools → Application → Local Storage | `longform_db` key present after first load | [P] | |
| 1.4 | DevTools → Application → Local Storage | `longform-settings` key present | [P] | |
| 1.5 | Observe default route | Library page renders with "Longform" heading | [P] | |
...
| # | Step | Expected | Status | Notes |
|---|------|----------|--------|-------|
| 2.1 | Observe header | "Longform" title · Global Refs · Corpus · Settings · "+ New Article" | [P] | |
...
- **Image data URLs:** Large images inserted as data URLs inflate the `longform_db` localStorage key significantly. No size warning is shown; no compression is applied. Consider using URL references instead of file uploads for large images.
