# Longform Web App — Session Context

**Project:** `/home/k/Claude_Projects/longform-web`
...
- sql.js (SQLite WASM) — DB exported to localStorage as base64 (`longform_db`)
...
| B-IMG1 | Image file uploads embedded base64 data URLs in markdown which broke the markdown tokenizer (URL too long). Fixed via `longform-img://uuid` scheme — images stored separately in localStorage, resolved at render time in `urlTransform`. **Not yet retested.** | Fix applied, needs retest |
...
- Image upload: now stores data URL in localStorage as `longform-img-{uuid}`, inserts short `longform-img://uuid` reference in markdown (fixes tokenizer breakage)
- `urlTransform` resolves `longform-img://uuid` → data URL at render time
...
- `saveBackupSlot()`: collects DB + settings + all `longform-img-*` images → rotates 3 slots
- `getBackupSlots()`, `clearBackupSlots()`, `downloadBackupSlot()`, `restoreFromPayload()`

### `src/utils/imageStore.ts` (new)
- `storeImage(dataUrl)` → returns UUID, saves to `localStorage['longform-img-{uuid}']`
- `resolveImageUrl(url)` → resolves `longform-img://uuid` → data URL
...
- **Images in markdown**: never embed base64 directly — use `longform-img://uuid` scheme
