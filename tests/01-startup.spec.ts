...
test('1.3 likhatu_db key present after load', async ({ page }) => {
  await waitForApp(page);
  const key = await page.evaluate(() => localStorage.getItem('likhatu_db'));
  expect(key).not.toBeNull();
});

test('1.4 likhatu-settings key written after first change', async ({ page }) => {
  await waitForApp(page);
  // Trigger a settings write by navigating to settings and toggling theme
  await page.goto('/settings');
  await page.getByRole('button', { name: 'Dark' }).click();
  const key = await page.evaluate(() => localStorage.getItem('likhatu-settings'));
  expect(key).not.toBeNull();
});
